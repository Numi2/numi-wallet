...
actor PairingChannel {
    ...
    func makePresenceAssertion(
        for session: PeerTrustSession,
        ttl: TimeInterval = 90
    ) async throws -> PeerPresenceAssertion {
        guard session.isActive else {
            throw WalletError.peerTrustExpired
        }

        let localVerifyingKey = try await keyManager.ensurePeerIdentityPublicKey()
        guard localVerifyingKey == session.peerVerifyingKey else {
            throw WalletError.invalidPeerTrustSession
        }

        let issuedAt = Date()
        let expiresAt = min(issuedAt.addingTimeInterval(ttl), session.expiresAt)
        let unsigned = UnsignedPeerPresenceAssertion(
            id: UUID(),
            sessionID: session.id,
        ...
        let payload = try JSONEncoder().encode(unsigned)
        let signature = try await keyManager.signPeerPayload(payload)
        return PeerPresenceAssertion(
            id: unsigned.id,
            sessionID: unsigned.sessionID,
            peerDeviceID: unsigned.peerDeviceID,
            peerRole: unsigned.peerRole,
            transport: unsigned.transport,
            capabilities: unsigned.capabilities,
            proximityEvidence: unsigned.proximityEvidence,
            trustLevel: unsigned.trustLevel,
            nearbyVerification: unsigned.nearbyVerification,
            transcriptFingerprint: unsigned.transcriptFingerprint,
            peerVerifyingKey: unsigned.peerVerifyingKey,
            appAttested: unsigned.appAttested,
    ...
    func verify(_ assertion: PeerPresenceAssertion, for session: PeerTrustSession) async throws -> Bool {
        guard assertion.isActive,
              assertion.matches(session: session) else {
            return false
        }
